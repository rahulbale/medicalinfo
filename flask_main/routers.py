...
def ValuePredictor(to_predict_list, size):
    ...
    if (size == 8):  # Diabetes
        ...
        loaded_model = joblib.load("flask_main/models/kidney_model")
        result = loaded_model.predict(to_predict)
    elif (size == 7):  # liver
...
def result():
    if request.method == 'POST':
        ...
        if (len(to_predict_list) == 12):  # Cancer
            ...
            return (render_template("result.html", prediction=prediction))
        elif (len(to_predict_list)==7): #kidney
            result = ValuePredictor(to_predict_list,7)
            if (int(result) == 0):
                prediction = "Congrulation! You DON'T have Kidney disease."
            else:
                prediction = "Oops! You have Kidney disease."
            return (render_template("result.html", prediction=prediction))
